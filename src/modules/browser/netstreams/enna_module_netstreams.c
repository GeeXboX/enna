...
static Evas_List *
browse_streams_list (netstreams_priv_t *data)
{
  Evas_List *list = NULL;
  Evas_List *l;
...
static Evas_List *
parse_netstream (const char *path, netstreams_priv_t *data)
{
  FILE *f;
...
static Evas_List *
browse_up (const char *path, netstreams_priv_t *data, char *icon)
{
  if (!path)
...
static Evas_List *
browse_up_music (const char *path)
{
  return browse_up (path, mod->music, "icon/music");
}

static Evas_List *
browse_up_video (const char *path)
{
  return browse_up (path, mod->video, "icon/video");
}

static Evas_List *
browse_down (netstreams_priv_t *data)
{
  if (!data->prev_uri)
    return browse_streams_list (data);
...
static Evas_List *
browse_down_music (void)
{
  return browse_down (mod->music);
}

static Evas_List *
browse_down_video (void)
{
...
{
  return
    enna_vfs_create_directory ((char *) mod->music->uri,
                               (char *) ecore_file_file_get (mod->music->uri),
                               (char *) evas_stringshare_add ("icon/music"),
                               NULL);
}
...
{
  return
    enna_vfs_create_directory ((char *) mod->video->uri,
                               (char *) ecore_file_file_get (mod->video->uri),
                               (char *) evas_stringshare_add ("icon/video"),
                               NULL);
}

static void
class_init (const char *name, netstreams_priv_t **priv,
   ...
   for (l = cfgdata->pair; l; l = l->next)
   {
     ...
     if (pair->key && !strcmp (pair->key, key))
     {
       ...
       data->config_netstreams =
         evas_list_append (data->config_netstreams, stream);       
     }
   }
}
...
module_init(Enna_Module *em)
{
  ...
  class_init ("netstreams_music", &mod->music,
              ENNA_CAPS_MUSIC, &class_music, "stream_music");
  class_init ("netstreams_video", &mod->video,
              ENNA_CAPS_VIDEO, &class_video, "stream_video");
}
